...
//         break;
//       default:
//         errorinfo("Unknown size %d", segy_key[i].size);
//         break;
//     }
//   }
// }
...
/*< Extract a SEGY binary header key value >*/
{
  for (int i = 0; i < SF_BHKEYS; i++) {
...
/*< Convert an integer trace[nk] to buffer buf >*/
{
  ...
  for (i = 0; i < nk; i++) {
    if (i < SF_NKEYS && 2 == standard_segy_key[i].size) {
      insert2(trace[i], buf);
      buf += 2;
    } else {
      insert4(trace[i], buf);
...
int writesegyheader(FILE* file, const char* content, int isskip,
                    int isebccode) {
  if (isskip) {
    fseek(file, SF_EBCBYTES, SEEK_SET);
    return 1;
  }
  char ahead[SF_EBCBYTES];
  memset(ahead, ' ', SF_EBCBYTES); // Fill with spaces first
  
  if (content != NULL) {
    size_t content_len = strlen(content);
    // Copy only up to the minimum of content length and SF_EBCBYTES
    strncpy(ahead, content, content_len < SF_EBCBYTES ? content_len : SF_EBCBYTES);
  }
  
  if (isebccode) {
    // convert ascii to ebcdic
    asc2ebc(SF_EBCBYTES, ahead);
    return fwrite(ahead, SF_EBCBYTES, 1, file);
  } else {
    return fwrite(ahead, SF_EBCBYTES, 1, file);
  }
}
